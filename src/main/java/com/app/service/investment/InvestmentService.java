...
public class InvestmentService {
    ...
    public List<InvestmentData> getInvestmentData() {
        val isAdmin = userService.getCurrentUser().filter(u -> u.getRoles().stream().map(r -> r.getName()).toList().contains(ADMIN.name())).isPresent();
    ...
    public void updateInvestmentData(List<InvestedInfo> investedInfos) {
        val emailIds = investedInfos.stream().map(infos -> infos.getEmailId().trim()).collect(Collectors.toList());
        val existingInvestmentData = investmentDataRepository.findByUserEmailIdIn(emailIds);
        val existingUserInvestmentDataMap = existingInvestmentData.stream().collect(Collectors.toMap(i -> i.getUser().getEmailId(), Function.identity()));
        val usersMap = userRepository.findAll().stream().collect(Collectors.toMap(i -> i.getEmailId(), Function.identity()));

        val investmentData = investedInfos.stream().map(info -> {
            if(existingUserInvestmentDataMap.get(info.getEmailId().trim()) != null) {
                ...
                invData.setLastEditedBy(userService.getCurrentUser().map(u -> u.getEmailId()).orElse("UNKNOWN"));
                return invData;
            } else {
                return InvestmentData.builder()
                        ...
                        .lastEditedBy(userService.getCurrentUser().map(u -> u.getEmailId()).orElse("UNKNOWN"))
                        .build();
            }
    ...
    private User createAndGetUser(String emailId, Map<String, User> userMap) {
        if(userMap.get(emailId)!=null) {
            ...
            return userRepository.save(User.builder().emailId(emailId).isActive(false).password("NONE")
